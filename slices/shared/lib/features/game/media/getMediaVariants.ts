...
export const getMediaVariants = (details: InvestigatorDetails) => {
	...
	const variants = media.variants.map(
		(variant): InvestigatorDetailItem<InvestigatorVariant> => {
			const { type, name, image } = variant;
			const id = getMediaVariantId(variant);

			const variantDetails =
				alternate.find(({ investigator }) => investigator.code === id) ||
				details;

			const { story } = variantDetails;
			const icon =
				story.icon === "parallel" ? "cardicons-parallel" : story.icon;
