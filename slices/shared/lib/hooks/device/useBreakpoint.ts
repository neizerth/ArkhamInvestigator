...
import type { BreakpointSize, DeviceBreakpointType, DeviceOrientation, DeviceType } from "@shared/model";
import { useWindowDimensions } from "react-native";
import { useMediaQuery } from 'react-responsive'
import { breakpoints, breakpointSizes, deviceTypes } from "../../../config";
...
export const useBreakpoint = (): BreakpointInfo => {
  ...
  for (const deviceType of deviceTypes) {
    for (const breakpointSize of breakpointSizes) {
      ...
      if (width <= size) {
        const breakpoint = `${deviceType}${capitalize(breakpointSize)}` as DeviceBreakpointType;
