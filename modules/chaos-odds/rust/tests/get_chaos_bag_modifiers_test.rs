...
fn minus_four_probability_two_curse_reveals() {
    ...
    // Expected: draw curse (2/4) then reveal curse (1/3) = 2/12 = 1/6
    // But actual result is 1/12, which suggests only one path is counted
    // This might be due to how grouping handles identical tokens
    // After removing approximation, the calculation gives 1/12
    let expected = (1.0 / 4.0) * (1.0 / 3.0); // draw curse then curse (one path)
