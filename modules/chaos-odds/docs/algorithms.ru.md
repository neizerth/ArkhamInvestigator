...
**Особый случай — сложность 0:**
На сложности 0 вероятность успеха не зависит от навыка. Нужно вычислить только `1 - вероятность_автопровала`. Автопровал — это жетон, который автоматически проваливает проверку (обычно это жетон "skull" или "cultist" с особым свойством). Если автопровала нет, вероятность успеха = 100%.
...
2. Вытягиваем "0" (вероятность 2/3)
   - reveal=0, это жетон без добора, больше ничего не нужно
...
Нужно хранить много информации компактно. Можно использовать массивы:
```rust
available_counts: [3, 2, 1, 0, ...]  // Сколько жетонов каждого типа доступно
drawn_counts: [1, 0, 2, 0, ...]       // Сколько жетонов каждого типа вытянуто
```

Но это занимает много памяти. Используем **bit packing**.

### Что такое bit packing

Вместо хранения чисел в отдельных ячейках, "упаковываем" их в одно большое число. Для извлечения данных используем битовые маски.
...
**Битовая маска** — это просто число, где каждый бит (0 или 1) означает какое-то свойство. Если бит = 1, свойство активно, если = 0 — неактивно.

**Пример:**
- У нас есть 4 группы жетонов: 0, 1, 2, 3
- Группы 0, 1 и 3 доступны (есть жетоны), группа 2 недоступна
- Маска: `1011` (в двоичном) = 11 (в десятичном)
  ```
  Позиция:  3  2  1  0
  Маска:    1  0  1  1
            ↓  ↓  ↓  ↓
  ...
  Доступна: ✓  ✗  ✓  ✓
  ```
  - Позиция 0 = 1 → группа 0 доступна
  - Позиция 1 = 1 → группа 1 доступна
  - Позиция 2 = 0 → группа 2 недоступна
  - Позиция 3 = 1 → группа 3 доступна
...
- **Хеш** — это число, полученное из данных. Одинаковые данные дают одинаковый хеш
- **Хеширование** — процесс превращения данных в хеш
- **Хеш-функция** — функция, которая превращает данные в хеш

**Пример:**
- У нас есть состояние: state1=100, state2=200
- Хеш-функция: `hash = (state1 * 17 + state2) % 1000`
- Результат: `hash = (100 * 17 + 200) % 1000 = 1900 % 1000 = 900`
- Теперь можем использовать 900 как индекс в массиве

**Зачем:**
- Быстро найти, встречали ли мы это состояние раньше
- Вместо сравнения всех данных, сравниваем одно число
...
**Пример:**
- Было: `probability = count / total` (деление каждый раз)
- Стало: `reciprocal = 1.0 / total` (один раз), затем `probability = count × reciprocal` (умножение быстрее)

### Таблица вероятностей

**Что такое таблица:**
Таблица — это массив, где можно быстро найти значение по индексам. Как телефонный справочник: знаешь имя — находишь номер.
...
Чтобы найти 2×3, смотрим на строку 2, столбец 3 → получаем 6
```
...
- Вместо вычисления 3/5 каждый раз, просто берём значение из таблицы
...
- Помещается в L2 кэш процессора (256 KB - 1 MB)

**Что такое L2 кэш:**
L2 кэш — это быстрая память внутри процессора (256 KB - 1 MB). Доступ к ней быстрее, чем к основной памяти. Если данные в L2 кэше, процессор получает их быстро.
...
Хеш-функция превращает данные (состояние) в число (хеш). Одинаковые данные всегда дают одинаковый хеш. Это позволяет быстро находить, встречали ли мы это состояние раньше.
...
- Результат — число от 0 до 127,999, которое используем как индекс в массиве
...
**Локальный кэш (L1):**
- **Что такое L1 кэш:** L1 кэш — самая быстрая память процессора (32-64 KB). Доступ к ней почти мгновенный (1-2 такта процессора)
...
**Что такое L1 кэш:**
L1 кэш — самая быстрая память процессора (32-64 KB). Доступ к ней почти мгновенный (1-2 такта процессора). Если данные в L1 кэше, процессор получает их очень быстро.

**Преимущества:**
- Нулевые выделения памяти в куче
...
Когда несколько потоков работают с одними данными, нужно координировать доступ, чтобы не испортить данные. Это делается через блокировки (locks) — механизм, который позволяет только одному потоку работать с данными в момент времени. Но блокировки медленные: потоки ждут друг друга.

**Что такое локальный контекст:**
Локальный контекст — это набор данных, которые принадлежат одному потоку. Каждый поток имеет свою копию данных, поэтому потоки не мешают друг другу и не нужны блокировки.
...
**Что такое начальные состояния (корни):**
Начальные состояния (или корни) — это состояния, с которых начинается обработка. Например, если вытянули жетон с добором, создаём начальное состояние: вытянули этот жетон, нужно вытянуть ещё N жетонов.
...
Вместо обработки всех начальных состояний последовательно, разбиваем их на чанки (порции):
...
**Логика:**
- Если начальных состояний мало (≤ количества потоков): маленькие порции для максимальной гибкости
- Если начальных состояний много: большие порции (2-4) для лучшей локальности кэша

#### Шаг 2: Work-stealing (воровство работы)

**Что такое work-stealing:**
Work-stealing (воровство работы) — алгоритм распределения работы между потоками. Если у потока закончились задачи, он "крадёт" задачи у других потоков. Это обеспечивает равномерную загрузку всех потоков.

**Зачем нужна библиотека Rayon:**
Rayon — библиотека для параллельной обработки данных в Rust. Она автоматически распределяет работу между потоками, используя алгоритм work-stealing. Нам не нужно вручную управлять потоками — Rayon делает это за нас.
...
1. **Компактное представление данных** — bit packing вместо массивов
